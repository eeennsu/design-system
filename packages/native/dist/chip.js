...
export const Chip = ({ label, selected = false, disabled = false, onPress, className, ref }) => (_jsx(Pressable, { ref: ref, accessibilityRole: "button", accessibilityLabel: label, accessibilityState: { selected, disabled }, disabled: disabled, hitSlop: hitSlop, 
    ...
    onPress: onPress && (() => onPress()), className: cn("flex-row items-center justify-center border px-4 py-2 rounded-full", selected
        ? "border-brand bg-brand hover:bg-brand-hover active:bg-brand-hover"
        : "border-border bg-surface hover:bg-surface-hover active:bg-surface-hover", disabled && "opacity-50", className), children: _jsx(RNText, { maxFontSizeMultiplier: 1.5, className: cn("text-sm", selected ? "text-fg-on-brand" : "text-fg"), children: label }) }));
